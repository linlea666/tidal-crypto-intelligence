...
def main():
    ...
    generation = next((g for g in ["v2.4", "v2.3", "v2.2", "v2.1", "v2"] if (PRIVATE / f"soak-{g}-start.json").exists()), "v1")
    ...
    summary["identityMismatchSamples"] = [s["utc"] for s in samples if generation in ("v2.2", "v2.3", "v2.4") and s.get("deployedVersion") != expected_identity]
    ...
    if generation != "v1":
        ...
        summary["bookBaseline"]=collector.get("bookBaseline")
    ...
    try:
        with client.open(req, timeout=15) as response:
            json.load(response)
        summary["apiMs"] = {}
        extra = ["signals?asset=BTC", "wallet-trends?asset=BTC", "etf?asset=BTC", "etf?asset=ETH", "studies?asset=BTC"] if generation in ("v2.2", "v2.3", "v2.4") else []
        for endpoint in (["large-orders?asset=BTC&limit=50", "signals?asset=ETH"] if generation in ("v2.3","v2.4") else []) + extra + (["activity?asset=BTC&hours=1"] if generation in ("v2.1","v2.2","v2.3","v2.4") else []) + ["levels?asset=BTC", "levels?asset=ETH", "flow?asset=BTC&hours=1", "candles?asset=BTC&hours=24", "whales?asset=BTC&limit=50", "derivatives?asset=ETH"]:
