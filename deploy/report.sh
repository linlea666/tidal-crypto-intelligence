...
for name in files[-4:]:
 for line in open(name):
  try:
   ...
   item['collector']={k:c[k] for k in ['at','uptimeSeconds','heapBytes','goroutines'] if k in c}
   item['collector']['markets']={a:{'validBooks':m.get('validBooks'),'totalBooks':m.get('totalBooks'),'freshWhales':m.get('freshWhales'),'observedWhales':m.get('observedWhales'),'validDerivatives':sum(1 for d in m.get('derivatives',[]) if d.get('valid')),'rates':m.get('rates',[])} for a,m in c.get('markets',{}).items()}
   records.append(item)
