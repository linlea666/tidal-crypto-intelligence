...
flock -n 8 || exit 0
docker run --rm --memory 192m --cpus .5 -v "$root/letsencrypt:/etc/letsencrypt" -v "$root/acme:/var/www/acme" -v "$root/certbot-logs:/var/log/letsencrypt" certbot/certbot:v5.4.0 renew --quiet
